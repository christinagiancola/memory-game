...
function checkForMatch() {
    ...
    if ( cardsChosen[0] === cardsChosen[1] ) {                        // test is the pair matches
    ...
    if (cardsWon.length === cardArray.length/2) {                     // how to tell if all the matches have been found
...
function flipCard() {
    ...
    if (cardsChosenId.length === 1 && cardsChosenId[0] === cardId) {return} // if you click the same image twice, you will not get a match
    ...
    if (cardsChosenId.length === 2) {                                       // when there are two elements in the array, runs "check for match"
